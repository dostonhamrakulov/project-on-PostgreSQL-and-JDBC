 ...
 * @version $Id:$
 */
public final class PersonalPizza extends Pizza
{

    private TreeMap<Long, Ingredient> ingredients;

    /**
     * Erzeuge Pizza fuer die Bestellung
     *
     *
     * @param aRequest Bestellanfrage
     * @param pizzaId Die Nummer der Pizze
     * @param ingredientIds Liste der zusaetzlichen Zutaten
     *
     * @see
     */
    public PersonalPizza(HttpServletRequest aRequest, String pizzaId, String ingredientIds)
    {
        ...
        IngredientList ingredientList = (IngredientList) theSession.getAttribute("ingredientList");

        System.out.println(pizzaId);

        Pizza pizza = pizzaList.getPizza(Long.valueOf(pizzaId.trim()));
        ...
        while (st.hasMoreElements())
        {
            ...
            Long ingredientId = Long.valueOf(s_ingredientId.trim());
    ...
    /**
     *
     * Berechne den Preis der Pizza. Der Preis berechnet sich aus dem Preis der
     * gewaehlten Grundpizza und den weiteren Zustaten.
     *
